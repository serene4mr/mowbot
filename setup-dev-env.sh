...
# Disable Docker restart when running in container environment
if [ -f /.dockerenv ] || [ -f /run/.containerenv ] || [ ! -d /run/systemd/system ] || ! ps -p 1 -o comm= 2>/dev/null | grep -q "^systemd\|^init$"; then
